...
const faqData = [
  {
    ...
    questions: [
      ...
      {
        q: 'When does NEET UG 2026-27 counselling start?',
        a: 'NEET UG 2026-27 counselling typically begins 4–6 weeks after the NEET UG results are declared. MCC conducts 4 rounds: Round 1, Round 2, Mop-Up Round, and Stray Vacancy Round. Exact dates are announced by MCC on their official website. Our team monitors these dates in real time and will notify you immediately.',
      },
      {
          ...
          {[{ label: 'All Questions', icon: '💡' }, ...faqData.map(c => ({ label: c.category, icon: c.icon }))].map((cat, i) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold border transition-all duration-200 ${isActive
          ...
          {displayed.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  {/* Number badge */}
                  <span className={`flex-shrink-0 w-7 h-7 rounded-lg text-xs font-black flex items-center justify-center transition-all duration-200 ${isOpen ? 'bg-[#0a3d62] text-white' : 'bg-slate-100 text-slate-400 group-hover:bg-[#eff8ff] group-hover:text-[#1a6fa8]'
                    }`}>
                    {(item.qIndex + 1).toString().padStart(2, '0')}
                  </span>

                  <span className={`flex-1 text-sm font-semibold leading-snug transition-colors duration-200 ${isOpen ? 'text-[#0a3d62]' : 'text-slate-700 group-hover:text-[#0a3d62]'
                    }`}>
                    {item.q}
                  </span>
                  ...
                  {/* Chevron */}
                  <div className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-300 ${isOpen ? 'bg-[#0a3d62] rotate-180' : 'bg-slate-100 group-hover:bg-[#eff8ff]'
                    }`}>
